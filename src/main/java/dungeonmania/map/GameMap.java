...
import dungeonmania.entities.movementListeners.ActionOnDestroy;
import dungeonmania.entities.movementListeners.ActionOnMoveAway;
...
public class GameMap {
  ...
  private void triggerMovingAwayEvent(Entity entity) {
    ...
    getEntities(entity.getPosition()).forEach(e -> {
      if (e != entity && e instanceof ActionOnMoveAway)
        callbacks.add(() -> ((ActionOnMoveAway) e).onMovedAway(this, entity));
    });
  ...
  private void triggerOverlapEvent(Entity entity) {
    ...
    getEntities(entity.getPosition()).forEach(e -> {
      if (e != entity && e instanceof ActionOnOverlap) {
        overlapCallbacks.add(() -> ((ActionOnOverlap) e).onOverlap(this, entity));
      }
    });
  ...
  public Position dijkstraPathFind(Position src, Position dest, Entity entity) {
    ...
    while (!q.isEmpty()) {
      ...
      List<Position> neighbours = curr.getCardinallyAdjacentPositions().stream().filter(p -> !visited.containsKey(p))
          .filter(p -> !nodes.containsKey(p) || nodes.get(p).canMoveOnto(this, entity)).collect(Collectors.toList());
  ...
  public void destroyEntity(Entity entity) {
    removeNode(entity);
    if (entity instanceof ActionOnDestroy) {
      ((ActionOnDestroy) entity).onDestroy(this);
